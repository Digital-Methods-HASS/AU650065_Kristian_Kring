...
## Task 1 Relabel the column name
...
Western_Europe
```
First I relabel the column, by giving it a new value called "homicides". After that I call the new value.

## Task 2 Creating a new visualisation
...
In this task I use the geom_line, instead of the ggplot, to visulaize the data for a long-term trend.

## Task 3 Changing the facet layout
3) Can you change the facet layout to two columns and three rows so that the trends are more visible? 

```{r}
ggplot(data = Western_Europe) + 
  geom_line(mapping = aes(x = Year, y = Homicides, color = Entity)) +
  facet_wrap(~ Entity, nrow = 3) +
  labs(x = "Year",
       y = "Homicides per 100,000",
       title = "Homicide rate in Western Europe from 1300-2000")
...
```
I use the facet_wrap function to tell that I want the facet layout in three rows.

## Task 4 Moving the legend below the facet
...
ggplot(data = Western_Europe) + 
  geom_line(mapping = aes(x = Year, y = Homicides, color = Countries)) +
...
First I rename "Entity" to countries, and afterwards I use the "theme" function to move the legend to the bottom. 


## Task 5 providing informative chunk-names
...
I use the r Sys.Date() to generate an automatically timestamp, after that I use toc_float to make a floating table of contents.
